...
def upgrade() -> None:
    # Check if table already exists (created in 001_initial)
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    
    if 'artist_analyses' not in inspector.get_table_names():
        op.create_table(
            'artist_analyses',
    ...
    existing_indexes = [idx['name'] for idx in inspector.get_indexes('artist_analyses')]
    
    if 'ix_artist_analyses_artist_name' not in existing_indexes:
        op.create_index('ix_artist_analyses_artist_name', 'artist_analyses', ['artist_name'])
    if 'ix_artist_analyses_created_at' not in existing_indexes:
