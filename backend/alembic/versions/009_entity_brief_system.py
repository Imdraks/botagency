...
def upgrade() -> None:
    # Entity types enum
    entity_type = postgresql.ENUM('PERSON', 'ORGANIZATION', 'TOPIC', name='entitytype', create_type=False)
    entity_type.create(op.get_bind(), checkfirst=True)
    
    # Objective types enum
    objective_type = postgresql.ENUM('SPONSOR', 'BOOKING', 'PRESS', 'VENUE', 'SUPPLIER', 'GRANT', name='objectivetype', create_type=False)
    objective_type.create(op.get_bind(), checkfirst=True)
    
    # Contact types enum
    contact_type = postgresql.ENUM('EMAIL', 'FORM', 'BOOKING', 'PRESS', 'AGENT', 'MANAGEMENT', 'SOCIAL', 'PHONE', name='contacttype', create_type=False)
    ...
    op.create_table(
        ...
        sa.Column('entity_type', sa.Enum('PERSON', 'ORGANIZATION', 'TOPIC', name='entitytype'), nullable=False, index=True),
    ...
    op.create_table(
        ...
        sa.Column('source_config_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('source_configs.id', ondelete='SET NULL'), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('opportunity_type', sa.Enum('SPONSOR', 'BOOKING', 'PRESS', 'VENUE', 'SUPPLIER', 'GRANT', name='objectivetype'), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column('entity_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('entities.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('contact_type', sa.Enum('EMAIL', 'FORM', 'BOOKING', 'PRESS', 'AGENT', 'MANAGEMENT', 'SOCIAL', 'PHONE', name='contacttype'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column('entity_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('entities.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('objective', sa.Enum('SPONSOR', 'BOOKING', 'PRESS', 'VENUE', 'SUPPLIER', 'GRANT', name='objectivetype'), nullable=False),
